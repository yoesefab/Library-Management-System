...
function MonthCalendar({ month, range, onSelect, onPrevious, onNext, showPrevious, showNext }) {
  ...
  return (
    <section className="date-range-month">
      <header>
        {showPrevious ? (
          <button aria-label="Mois précédent" onClick={onPrevious} type="button">
        ...
        {showNext ? (
          <button aria-label="Mois suivant" onClick={onNext} type="button">
      ...
      <div aria-label={`${MONTHS[month.getMonth()]} ${month.getFullYear()}`} className="date-range-days" role="grid">
        {days.map((day) => {
          ...
          return (
            <button
              aria-label={day.toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" })}
...
export function DateRangePicker({ label, onChange, value }) {
  ...
  const [viewMonth, setViewMonth] = useState(
    new Date(committedRange.to.getFullYear(), committedRange.to.getMonth() - 1, 1),
  );

  useEffect(() => {
    const nextRange = parseRange(value);
    setDraftRange(nextRange);
    setViewMonth(new Date(nextRange.to.getFullYear(), nextRange.to.getMonth() - 1, 1));
  }, [value]);
  ...
  const selectDay = (day) => {
    ...
    setDraftRange(day < draftRange.from ? { from: day, to: draftRange.from } : { from: draftRange.from, to: day });
    setSelectingEnd(false);
  };
  ...
  const selectedPreset = PRESETS.find(([name]) => {
    ...
    return sameDay(preset.from, draftRange.from) && sameDay(preset.to, draftRange.to);
  })?.[0];

  return (
    <div className="dashboard-filter dashboard-filter--period date-range-picker" ref={rootRef}>
      <span>{label}</span>
      <button
      ...
      {isOpen ? (
        <div aria-label="Choisir une période" className="date-range-popover" id={popoverId} role="dialog">
            ...
            <label>
              ...
              <input
                ...
                onChange={(event) => {
                  ...
                  setDraftRange({ from, to: from > draftRange.to ? from : draftRange.to });
                }}
                type="date"
            ...
            <label>
              ...
              <input
                ...
                onChange={(event) => {
                  ...
                  setDraftRange({ from: to < draftRange.from ? to : draftRange.from, to });
                }}
                type="date"
