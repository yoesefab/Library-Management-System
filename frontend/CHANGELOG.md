...
- Alignement des comparaisons en bas des cartes d’indicateurs du tableau de bord.
- Agrégation mensuelle des courbes du tableau de bord sur les douze mois de données de démonstration.
- Dimensionnement des axes des courbes selon les données réelles et chargement du détail des commandes avant de remplir leur tableau.
- Connexion de l’authentification à la session Spring Security et au jeton CSRF.
- Lecture des produits, stocks, mouvements, commandes, indicateurs, imports,
  alertes, prévisions, paramètres et événements d’audit depuis les API.
- Persistance des opérations métier et téléchargement des rapports, sans
  modification de la structure visuelle existante.

### Adaptation Maarif Analytics

- Placement du filtre de période des commandes à côté de la recherche. Badges de statut harmonisés avec les classes de couleurs claires/sombres shadcn : rouge doux pour les erreurs et inactivités, vert pour les succès, bleu/ciel/violet pour les étapes et ambre pour les avertissements.
- Suppression du bouton et du formulaire de création de commande manuelle, ainsi que de la logique locale associée ; consultation, filtres et export des commandes conservés.
- Refonte des dialogues de mouvement de stock et de commande manuelle avec les composants shadcn, grilles responsive, aperçu avant/après et confirmation conservée. Ville modifiable et requise pour le brouillon ; fermeture bloquée pendant l’enregistrement et suppression de la référence prévisualisée figée.
- Harmonisation des badges de statut des tableaux : vert clair pour les états positifs, ambre pour les avertissements, bleu pour le traitement en cours et rouge destructive pour les échecs/inactivités, avec variantes sombres communes.
- Filtres des rapports au format compact des barres de tableaux, avec recherche dans les options et recherche des rapports. Harmonisation des tableaux : en-têtes de 48 px, cellules de hauteur minimale 56 px et espacements communs de 16 px horizontalement / 12 px verticalement ; filtres adaptatifs. Renommage du lien Administration en Settings.
- Déplacement du journal d’audit vers la page dédiée /logs et ajout de Logs dans la navigation Gouvernance ; Administration affiche directement les paramètres système. Filtres, recherche et pagination du journal conservés.
- Migration de Prévisions, Rapports et Administration vers les composants shadcn et les couleurs du thème : cartes responsive, graphique et info-bulles lisibles en mode sombre, recherche produit au clavier, filtres Select et choix RadioGroup des rapports, formulaires et dialogues administratifs natifs. Les filtres et le type de rapport sont verrouillés pendant la génération pour préserver la cohérence du résultat.
- Migration des cinq étapes d’import des ventes vers les composants shadcn Card, Button, Badge, Alert et Input, avec couleurs de thème pour les panneaux, avertissements, résultats et progression ; prise en charge du mode sombre et affichage de la taille réelle du fichier sélectionné.
- Extension de la barre flottante native à l’inventaire, aux commandes et aux alertes : compteur, effacement et export CSV de la sélection filtrée, toutes pages confondues. Acquittement des nouvelles alertes et résolution des alertes ouvertes en lot après confirmation, sans modification du stock ; historique des mouvements conservé en lecture seule.
- Ajout au catalogue de la barre flottante de sélection native du dépôt amont : compteur français, effacement, export CSV des produits sélectionnés et désactivation locale avec confirmation, selon le rôle. Les actions portent sur la sélection correspondant aux filtres courants, toutes pages confondues.
- Utilisation de la variante Badge `destructive` pour le statut Désactivé du catalogue.
- Badge Actif du catalogue avec les classes `bg-green-50 text-green-700 dark:bg-green-950 dark:text-green-300`.
- Affichage des statuts Actif et Désactivé du catalogue avec le composant Badge natif shadcn, sans modifier l’alignement des colonnes.
- Rétablissement de l’alignement natif des cellules du catalogue, sans centrage personnalisé.
- Suppression de la colonne Seuil du catalogue (y compris le menu Affichage) ; seuil métier conservé pour les alertes.
- Réorganisation du dialogue de création produit en grille responsive à deux colonnes, avec description pleine largeur et une colonne sur mobile.
- Création produit dans un dialogue natif reprenant le formulaire utilisateur amont, sans quitter le catalogue : champs, validation et confirmation d’abandon conservés ; l’ancienne URL ouvre le même dialogue.
- Remplacement du sélecteur d’équipe en haut de la barre latérale par un logo et un nom statiques, sans menu « Add team » ni chevron.
- Suppression de l’avatar dans la barre supérieure des pages métier ; maintien du menu de profil dans la barre latérale.
- Réduction des espacements et hauteurs des cartes du dashboard et suppression de l’indicateur « Panier moyen » ; conservation de six indicateurs et des quatre graphiques.
- Alignement des cartes du dashboard sur le code amont : en-têtes avec icône à droite, valeurs en gras, comparaisons atténuées, grille responsive et espacements natifs ; suppression des classes héritées des cartes et conservation des métriques/graphiques.
- Suppression des trois tableaux du dashboard (meilleures ventes, stocks à risque et faible rotation), en conservant les indicateurs, graphiques et le sélecteur de période.
- Généralisation de la structure Tasks aux tables d’inventaire, mouvements, commandes, alertes, imports et audit, avec filtres, tri, colonnes, menus et pagination natifs.
- Migration des tableaux de détail produit/commande, de l’aperçu des ventes et des erreurs d’import vers le même composant partagé, sans CSS de tableau hérité ; conservation des données et confirmations métier.

- Reprise de la structure Tasks pour le catalogue produits : en-tête, filtres à facettes, tri, sélection, affichage des colonnes, menus d’action et pagination natifs, avec libellés français et thèmes shadcn.

- Reprise des écrans et interactions du prototype approuvé dans le shell responsive de shadcn-admin.
- Ajout des routes métier pour produits, inventaire, commandes, imports, alertes, prévisions, rapports et administration.
- Adoption de la navigation, du profil et de la connexion en français avec l’identité Maarif Culture.
- Conservation des données synthétiques marocaines, des montants en MAD, des dates Africa/Casablanca et des tables à défilement local sur mobile.
- Migration des cartes, boutons d’action et sélecteurs du tableau de bord vers les composants natifs `Card`, `Button` et `Select` de shadcn/ui.
- Ajout du thème sombre aux cartes, filtres, graphiques, info-bulles et tableaux du tableau de bord.
- Migration des tableaux du tableau de bord vers les primitives natives `Table`, `TableHeader`, `TableBody`, `TableRow`, `TableHead` et `TableCell` de shadcn/ui.
- Réorganisation des tableaux produits avec les meilleures ventes et les stocks à risque côte à côte, puis les produits à faible rotation sur une rangée complète.
- Amélioration de la lisibilité des tableaux avec une typographie agrandie et le centrage des colonnes secondaires.
- Augmentation de la hauteur des lignes et des espacements internes des cellules des tableaux du tableau de bord.
- Alignement visuel des tableaux du tableau de bord sur les tables natives de shadcn-admin, avec typographie, en-têtes, bordures et survols standards.
- Suppression des styles CSS hérités des tableaux au profit de la structure et des classes utilitaires exactes utilisées par les tables du dépôt shadcn-admin.
- Suppression de la surcharge Manrope du prototype afin d’utiliser la police Inter par défaut du dépôt shadcn-admin dans toute l’interface migrée.
- Simplification des filtres du tableau de bord en conservant uniquement la période, désormais placée dans la barre de navigation supérieure.
- Remplacement du sélecteur de période par une adaptation native shadcn du composant `date-range-picker-for-shadcn`, avec saisie segmentée, calendrier responsive et périodes rapides en français.
