...
public class CsvImportService {
    ...
    public Preview preview(MultipartFile file) throws IOException {
        ...
        String payload=new String(bytes,StandardCharsets.UTF_8);ImportJob job=jobs.save(new ImportJob(safeName(file.getOriginalFilename()),checksum,currentUser.required(),payload));
        int total=0;try(CSVParser parser=parse(payload)){Set<String> actual=parser.getHeaderMap().keySet();if(!actual.containsAll(HEADERS)){job.addError(1,"headers","MISSING_HEADERS","En-têtes requis manquants : "+missing(actual),null);job.validationComplete(1,1);return response(job);}
            Map<String,String> orderHeaders=new HashMap<>();Set<String> orderProducts=new HashSet<>();
            for(CSVRecord row:parser){total++;validateRow(job,row);validateOrderStructure(job,row,orderHeaders,orderProducts);}
    ...
    public Preview confirm(Long id) throws IOException {
        ImportJob job=jobs.findById(id).orElseThrow(()->new ResourceNotFoundException("Import introuvable : "+id));
    ...
    private void required(ImportJob job,CSVRecord row,int n,String field){if(row.get(field).isBlank())error(job,n,field,"REQUIRED","Valeur obligatoire.",null);}private void error(ImportJob j,int n,String f,String c,String m,String v){j.addError(n,f,c,m,v);}
