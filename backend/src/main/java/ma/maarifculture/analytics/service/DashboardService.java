...
public class DashboardService {
    ...
    public DashboardResponse dashboard(Instant start,Instant end,Long categoryId,String language,Long authorId,Long publisherId){
        ...
        long low=filtered.stream().filter(p->{int s=inventory.currentStock(p.getId());return s>0&&s<=p.getMinimumStockThreshold();}).count();long out=filtered.stream().filter(p->inventory.currentStock(p.getId())<=0).count();
        ...
        List<MetricPoint> categories=aggregate(sold,p->p.getCategory()==null?"Sans catégorie":p.getCategory().getName());List<MetricPoint> languages=aggregate(sold,Product::getLanguage);
        ...
        return new DashboardResponse(revenue,count,units,average,totalStock,value,low,out,turnover,remaining,best,slow,categories,languages,trend);
    }
    ...
    private List<MetricPoint> aggregate(List<OrderItem> sold,Function<Product,String> key){return sold.stream().collect(Collectors.groupingBy(i->key.apply(i.getProduct()),Collectors.reducing(BigDecimal.ZERO,OrderItem::getLineTotal,BigDecimal::add))).entrySet().stream().sorted(Map.Entry.<String,BigDecimal>comparingByValue().reversed()).map(e->new MetricPoint(e.getKey(),e.getValue())).toList();}
}
